...
export function mapProducto(row) {
  ...
  return {
    ...
    idTienda: row.idtienda ?? row.idTienda,
    idCategoria: row.idcategoria ?? row.idCategoria,
    idProveedor: row.idproveedor ?? row.idProveedor,
    idProductoMayorista: row.idproductomayorista ?? row.idProductoMayorista,
    nombre: row.nombre,
    descripcion: row.descripcion,
    precio: row.precio != null ? Number(row.precio) : row.precioventa != null ? Number(row.precioventa) : null,
    precioCosto: row.preciocosto != null ? Number(row.preciocosto) : row.precioCosto != null ? Number(row.precioCosto) : null,
    stock: row.stock,
    stockMinimo: row.stockminimo ?? row.stockMinimo,
    marca: row.marca,
    iva: row.iva != null ? Number(row.iva) : null,
    fechaVencimiento: row.fechavencimiento ?? row.fechaVencimiento,
    vendidosTotales: row.vendidostotales ?? row.vendidosTotales,
...
export function mapDetallePedido(row) {
  ...
  return {
    ...
    idProducto: row.idproducto ?? row.idProducto,
    idTienda: row.idtienda ?? row.idTienda,
    nombreProducto: row.nombre_producto ?? row.nombreProducto,
    imagenProducto: row.imagen_producto ?? row.imagenProducto,
    marcaProducto: row.marca_producto ?? row.marcaProducto,
    nombreTienda: row.nombre_tienda ?? row.nombreTienda,
    descripcionTienda: row.descripcion_tienda ?? row.descripcionTienda,
    cantidad: row.cantidad,
...
export function mapDetalleCompraProveedor(row) {
  ...
  return {
    ...
    idCompra: row.idcompra ?? row.idCompra,
    idProductoMayorista: row.idproductomayorista ?? row.idProductoMayorista,
    nombreItem: row.nombre_producto_mayorista ?? row.nombreitem ?? row.nombreItem,
    cantidad: row.cantidad,
